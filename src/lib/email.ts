...
// ─── Types ────────────────────────────────────────────────────────────────────
export type PlanId = 'gratis' | 'basico' | 'starter' | 'pro' | 'enterprise';
...
function buildWelcomeHtml(businessName: string, planId: PlanId, dashboardUrl: string): string {
  ...
  const isTrialWelcome = planId === 'gratis';

  const trialBanner = isTrialWelcome ? `
  ...
  const planBenefits: Record<PlanId, string[]> = {
    gratis: ['Código QR único para tu local', 'Hasta 40 pedidos por día', 'Cocina en tiempo real'],
    basico: ['Hasta 15 productos en tu menú', 'Código QR único para tu local', 'Hasta 40 pedidos por día', 'Cocina en tiempo real'],
...
// ─── Subscription expiring email HTML template ───────────────────────────────
function buildExpiringHtml(businessName: string, daysLeft: number, pricingUrl: string): string {
  ...
  const dayLabel = daysLeft === 1 ? '¡Último día!' : `${daysLeft} días`;
  const subject  = daysLeft === 1
    ? 'Hoy vence tu prueba — activa tu plan ahora 🔔'
    : `Tu prueba vence en ${daysLeft} días — no pierdas el acceso`;
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:24px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="background:#111827;padding:32px 40px;text-align:center;">
              ...
              <h1 style="margin:8px 0 0;font-size:26px;font-weight:800;color:#ffffff;letter-spacing:-0.5px;">
          ...
          <tr>
            <td style="padding:40px 40px 32px;">
              <p style="margin:0 0 20px;font-size:17px;color:#111827;font-weight:600;">
                Hola, <strong>${businessName}</strong> 👋
              </p>

              <!-- Urgency badge -->
              <div style="background:${urgencyBg};border:1px solid ${urgencyBorder};border-radius:12px;padding:16px 20px;margin:0 0 24px;text-align:center;">
                <p style="margin:0;font-size:22px;font-weight:800;color:${urgencyColor};">${dayLabel}</p>
                <p style="margin:6px 0 0;font-size:14px;color:${urgencyColor};opacity:0.85;">
                  ${daysLeft === 1 ? 'Tu periodo de prueba vence hoy.' : `Te quedan ${daysLeft} días de prueba gratuita.`}
                </p>
              </div>

              <p style="margin:0 0 24px;font-size:15px;color:#6b7280;line-height:1.6;">
                Cuando expire tu prueba, tu cuenta quedará <strong>sin plan activo</strong> con acceso restringido a 40 pedidos/día y sin inventario ni reportes.
                Activa tu plan ahora para mantener todo sin interrupciones.
              </p>

              <!-- What you keep -->
              <div style="background:#f3f4f6;border-radius:16px;padding:24px 28px;margin-bottom:28px;">
                <p style="margin:0 0 14px;font-size:14px;font-weight:700;color:#111827;text-transform:uppercase;letter-spacing:1px;">
                  Con un plan pagado conservas:
                </p>
                <ul style="margin:0;padding:0 0 0 4px;list-style:none;">
                  <li style="margin-bottom:8px;color:#374151;">✅ Pedidos ilimitados todos los días</li>
                  <li style="margin-bottom:8px;color:#374151;">✅ Inventario y stock en tiempo real</li>
                  <li style="margin-bottom:8px;color:#374151;">✅ Reportes y analíticas de ventas</li>
                  <li style="margin-bottom:8px;color:#374151;">✅ Caja y arqueo automático</li>
                  <li style="margin-bottom:8px;color:#374151;">✅ Soporte prioritario</li>
                </ul>
              </div>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
                    <a href="${pricingUrl}" style="display:inline-block;background:#dc2626;color:#ffffff;font-weight:700;font-size:16px;text-decoration:none;padding:16px 40px;border-radius:14px;letter-spacing:0.3px;">
...
export interface SubscriptionExpiringEmailParams {
  to: string;
  businessName: string;
  daysLeft: number;
  pricingUrl?: string;
...
export async function sendSubscriptionExpiringEmail({
  ...
  businessName,
  daysLeft,
  pricingUrl = `${process.env.NEXT_PUBLIC_APP_URL ?? 'https://ape.teolabs.app'}/#pricing`,
}: SubscriptionExpiringEmailParams): Promise<void> {
  const transporter = createTransporter();

  const subjectMap: Record<number, string> = {
    7: '⏰ Tu prueba vence en 7 días — activa tu plan',
    3: '⚠️ Tu prueba vence en 3 días — no pierdas el acceso',
    1: '🔔 Hoy vence tu prueba de Hungry Ape',
  };
  const subject = subjectMap[daysLeft] ?? `⏰ Tu prueba vence en ${daysLeft} días`;

  await transporter.sendMail({
    from: `"Hungry Ape 🦍" <${process.env.ZOHO_SMTP_USER}>`,
    to,
    subject,
    html: buildExpiringHtml(businessName, daysLeft, pricingUrl),
    text: `Hola ${businessName},\n\nTu prueba de Hungry Ape vence en ${daysLeft} día${daysLeft === 1 ? '' : 's'}.\n\nActiva tu plan ahora para no perder el acceso: ${pricingUrl}\n\n¿Dudas? Escríbenos a hola@teolabs.app`,
  });

  console.log(`✅ Expiring reminder sent to ${to} (daysLeft: ${daysLeft})`);
}
